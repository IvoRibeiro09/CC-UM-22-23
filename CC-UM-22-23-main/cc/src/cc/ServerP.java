...
public class ServerP {
    ...
    public void SPSR(Query q,Cache ca) {
        while (true) {
            System.out.println("!!!!!!!!!! espera conexão de SR!!!!!!!!!!!!!!");
            try {
                DatagramPacket dp = new DatagramPacket(buffer,buffer.length);
                dsocket.receive(dp);
                //log QR
                InetAddress SRIp = dp.getAddress();
                ...
                dp = new DatagramPacket(buffer,buffer.length,SRIp,porta);
    ...
    public void STSR(Cache c){
        while (true) {
            ...
            try {
                ...
                System.out.println("dominio recebido: " + parte[0]);
                System.out.println("query recebida: " + parte[1]);

                buffer = new byte[64000];
                String querydone = q.doQuery(query,cc);
                buffer = querydone.getBytes();
                dp = new DatagramPacket(buffer,buffer.length,SRIp,porta);
                System.out.println("resposta enviada: "+ querydone);
                dsocket.send(dp);
